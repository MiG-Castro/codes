...
// ============================================================================
#define SP_MS_TO_WAIT 5 // tiempo en ms para que RX_Thread duerma/espere nuevos pkts
    ...
    void processing_loop() {
        ...
        while (running) {
            std::vector<rx_pkt> batch;
            {
                ...
                process_cv.wait(lock, [this] { return !process_queue.empty() || !running; }); // Sleep-Wait
            ...
            for (const rx_pkt& pkt : batch) {event_detector.process_sample(buffers[pkt.p], pkt.no_sample, pkt.p);}
        }

...
public:
    ...
    void process_sample(IMUSample buffers[BUFFER_SIZE], uint32_t sample_idx) {
        if (!calibrated) return;
...
public:
    ...
    void stop() {
        ...
        // std::this_thread::sleep_for(std::chrono::milliseconds(100));
        // std::this_thread::sleep_for(std::chrono::milliseconds(500));
        // serial.send_command({0x7E, 0x00, 0x19, 0x7F}); // Print logs/info OFF
...
private:
    ...
    void parse_packet(const uint8_t* pkt, size_t len) {
        ...
        // Si esta no es la primera muestra y ya se creo la matriz de rotacion
        if (!tracker.first_packet && event_detector.is_calibrated()) {
            ...
            if (packet_num != expected) {
                ...
                std::cout <<"P"<<(int)peripheral_id<<" "<<(int)tracker.last_packet_num
                          <<"->"<<(int)packet_num<<" L="<<(int)lost_count<<std::endl;
                
                // En caso de perdidas marcar "valid" como falso junto e indicar el numero de muestra

                for (uint32_t p = expected; p != packet_num; p++) {
                    for (uint32_t offset = 0; offset < SAMPLES_PER_PACKET; offset++) {
                        uint32_t sample_num = (p * SAMPLES_PER_PACKET + offset) % BUFFER_SIZE;
                        buffers[peripheral_id][sample_num].sample_num = p * SAMPLES_PER_PACKET + offset;
        ...
        for (int i = 0; i < SAMPLES_PER_PACKET; i++) {
            ...
            sample.sample_num = packet_num * SAMPLES_PER_PACKET + i;
            ...
            if (peripheral_id == 0 && event_detector.is_calibrated()) {
                ...
                process_queue.push(sample.sample_num); 
                global_sample_index = sample.sample_num + 1;
            }
        }
    }
    ...
    void processing_loop() {
        ...
        while (running) {
            if (!event_detector.is_calibrated()) {
                ...
                else {for (uint32_t sample_idx : batch) {event_detector.process_sample(buffers[0], sample_idx);}}
            }
        }
...
int main(int argc, char** argv) {
    ...
    std::cout << "Segmentador: IAAC en Nodo 0" << std::endl;
    std::cout << "============================================\n" << std::endl;

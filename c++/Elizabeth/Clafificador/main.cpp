...
#include <cmath>
#include <cstdio>
#include <cstdint>  // NUEVO: para uint32_t/uint8_t/etc en cualquier plataforma
#include <algorithm> // NUEVO: para std::min (usado en processing_loop)
// Windows headers
#ifdef _WIN32
// NUEVO: evita que windows.h arrastre winsock.h (choca con winsock2.h) y que
// defina macros min/max (chocan con std::min/std::max)
...
#else
// NUEVO: headers POSIX equivalentes para compilar en macOS/Linux
#include <unistd.h>
#include <fcntl.h>
#include <termios.h>
#include <errno.h>
...
// NUEVO: alias para que el resto del codigo (escrito con tipos/funciones de
// Windows: DWORD, SOCKET, INVALID_SOCKET, closesocket) compile igual en
// macOS/Linux sin tener que tocarlo.
...
void habilitarANSI() {
#ifdef _WIN32 // NUEVO: en macOS/Linux la terminal ya soporta ANSI de forma nativa, no hace falta nada
...
// ============================================================================
// NUEVO: PUENTE THREAD-SAFE HACIA LA INTERFAZ
// mlp_classifier corre en el hilo de "processing_loop", pero RehabInterface
// (GLFW/ImGui) solo se puede usar desde el hilo principal (obligatorio en
// macOS). Por eso el resultado se "publica" aca y el hilo principal lo
// consume en su propio loop para dibujar la ventana.
// ============================================================================
std::atomic<bool> g_appRunning{true};

std::mutex g_feedbackMutex;
bool g_feedbackValue = true;
bool g_feedbackDirty = false;

void PublishFeedback(bool wellPerformed) {
    std::lock_guard<std::mutex> lk(g_feedbackMutex);
    g_feedbackValue = wellPerformed;
    g_feedbackDirty = true;
}

// Devuelve true si habia un resultado nuevo (y lo copia en 'value')
bool ConsumeFeedback(bool& value) {
    std::lock_guard<std::mutex> lk(g_feedbackMutex);
    if (!g_feedbackDirty) return false;
    value = g_feedbackValue;
    g_feedbackDirty = false;
    return true;
}
...
private:
    ...
    void get_attributes(uint32_t start_pos, uint32_t end_pos, int l, uint8_t p) {
        ...
        // NUEVO: en vez de escribir directo a consola, publicamos el resultado
        // para que el hilo principal lo muestre en RehabInterface.
        bool bien_realizado = (mlp_classifier(sensor) != 0);
        PublishFeedback(bien_realizado);
    }
};
...
// ============================================================================
#ifndef _WIN32
// NUEVO: convierte un baudrate (int) a la constante B... que pide termios
speed_t baudrate_to_speed(int baud) {
    switch (baud) {
...
private:
    ...
    HANDLE hSerial;
#else
    int fd; // NUEVO: en macOS/Linux el puerto serial se maneja con un file descriptor
#endif
...
#else
    // NUEVO: mismo constructor pero implementado con termios (macOS/Linux)
    SerialPort(const std::string& port, int baudrate = 230400) : fd(-1), port_name(port) {
        fd = open(port.c_str(), O_RDWR | O_NOCTTY | O_NONBLOCK);
        if (fd < 0) {
            std::cerr << "Error: No se pudo abrir " << port << " (" << strerror(errno) << ")" << std::endl;
            return;
        }
        ...
        if (tcgetattr(fd, &tty) != 0) {
            std::cerr << "Error obteniendo atributos del puerto" << std::endl;
            close(fd);
            fd = -1;
            return;
        }
        ...
        tty.c_cflag &= ~CRTSCTS;                      // sin control de flujo por HW
        ...
        tty.c_cc[VMIN] = 0;
        tty.c_cc[VTIME] = 1; // timeout de lectura ~100ms
...
#else
        if (fd >= 0) { // NUEVO: equivalente POSIX
            close(fd);
            std::cout << "Puerto serial cerrado" << std::endl;
        }
#endif
...
#else
        return fd >= 0; // NUEVO: en POSIX un fd valido es >= 0
#endif
    }
...
#else
        // NUEVO: equivalente POSIX de WriteFile
...
#else
        // NUEVO: equivalente POSIX de ReadFile
        ssize_t n = read(fd, buffer, len);
        if (n < 0) return 0; // timeout o sin datos
        return (DWORD)n;
#endif
    }
...
public:
    ...
    initialized(false) {
#ifdef _WIN32 // NUEVO: WSAStartup/WSACleanup solo existen en Winsock
        WSADATA wsaData;
...
int main(int argc, char** argv) {
    ...
    // NUEVO: RehabInterface (GLFW/ImGui) solo se puede usar desde el hilo
    // principal, asi que el hilo principal ya no se puede quedar bloqueado
    // en cin.get(). El ENTER para detener se espera en un hilo aparte.
    ...
    // NUEVO: loop del hilo principal. Dibuja el resultado (bien/mal) cuando
    // llega uno nuevo, y a los pocos segundos RehabInterface vuelve sola a
    // la pantalla de "tu turno, repeti el ejercicio" (ver SetFeedbackDuration).
    while (g_appRunning) {
        bool fresh;
        if (ConsumeFeedback(fresh)) interfaz.ShowResult(fresh);

        if (!interfaz.Tick()) { // el usuario cerro la ventana
            g_appRunning = false;
            break;
        }
    ...
    receiver.stop();
    if (stopThread.joinable()) stopThread.detach(); // NUEVO: puede seguir esperando el ENTER, no bloqueamos el cierre

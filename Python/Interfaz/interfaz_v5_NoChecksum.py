...
class WSNInterface:
    ...
    def toggle_scan(self):
        ...
        if not self.scan_active:
            self.send_raw_hex("7E 00 20 7F")
            self.scan_btn.config(text="Stop Scan")
            self.scan_active = True
        else:
            self.send_raw_hex("7E 00 21 7F")
    ...
    def send_disconnect(self):
        ...
        try:
            ...
            message = f"7E 01 22 {value_byte:02X} 7F"
    ...
    def send_params_config(self):
        ...
        try:
            ...
            message = bytes([0x7E, 0x0B, 0x23]) + payload + bytes([0x7F])
            self.serial_connection.write(message)
    ...
    def send_print_logs(self):
        ...
        if self.printLogs: 
            ...
            self.send_raw_hex("7E 00 18 7F")
        else: 
            self.printLogs = True
            self.send_raw_hex("7E 00 19 7F")
    
    def send_tx_uart(self):
        """Envía comando TxUART (0x18)"""
        self.send_raw_hex("7E 00 16 7F")
    
    def start_sampling(self):
        """Envía comando Start Sampling a periférico"""
        try:
            idx = self.get_periph_idx()
            message = f"7E 02 30 {idx:02X} AA 7F"
            self.send_raw_hex(message)
        except Exception as e:
    ...
    def stop_sampling(self):
        ...
        try:
            idx = self.get_periph_idx()
            message = f"7E 02 30 {idx:02X} FF 7F"
            self.send_raw_hex(message)
        except Exception as e:
            messagebox.showerror("Error", f"Error enviando stop sampling: {str(e)}")
    
    def send_stop_pkt(self):
        """Envía comando StopPkt con formato 7E 05 25 [idx StopPkt-4B] 7F"""
        try:
            ...
            message = bytes([0x7E, 0x05, 0x25]) + payload + bytes([0x7F])
            self.serial_connection.write(message)
    ...
    def send_sampling_config(self):
        ...
        try:
            ...
            message = bytes([0x7E, 0x05, 0x24]) + payload + bytes([0x7F])
            self.serial_connection.write(message)
    ...
    def send_feature_control(self, command):
        ...
        try:
            idx = self.get_periph_idx()
            message = f"7E 02 30 {idx:02X} {command:02X} 7F"
            self.send_raw_hex(message)
        except Exception as e:
    ...
    def send_command(self):
        ...
        try:
            ...
            message = bytes([0x7E, payload_len, cmd_type]) + payload + bytes([0x7F])
            self.serial_connection.write(message)

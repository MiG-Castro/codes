...
f = open("S2S3_5min_3mxp_v0.txt", "r")
            ...
            if len(line_list) == 20:
                if line_list[0] == "3":
                    ...
                    if len(S_xx) > 1 and S_xx[-1] != (S_xx[-2] + 1):
                        S_Loss = S_Loss + 1
                        Total_Error = "Conteo de Errores: S3 = " + str(S_Loss) + ", S2 = " + str(S2_Loss) + \
                                      ", Total = " + str(S_Loss + S2_Loss)
                        print("Error en S3, Ultima muestra = ", S_xx[-2], ", Nueva muestra = ", S_xx[-1],
                              ", Perdidas = ", (S_xx[-1] - S_xx[-2]))
                        print(Total_Error)
                    #else:
                    ...
                    if len(S2_xx) > 1 and S2_xx[-1] != (S2_xx[-2] + 1):
                        ...
                        Total_Error = "Conteo de Errores: S3 = " + str(S_Loss) + ", S2 = " + str(S2_Loss) + \
                                      ", Total = " + str(S_Loss + S2_Loss)
                        print("Error en S2, Ultima muestra = ", S2_xx[-2], ", Nueva muestra = ", S2_xx[-1],
                              ", Perdidas = ", (S2_xx[-1] - S2_xx[-2]))
                        print(Total_Error)
                    #else:

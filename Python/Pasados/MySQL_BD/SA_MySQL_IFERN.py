...
paciente = 39
Extremidad = "D"
ejercicio = "08-04-2022_07-32-52_IFEB_s2.txt"
...
info = [paciente, int(temp_info[2]), "IFERN", Extremidad]    # Paciente[0] Sensor[1] Ejercicio[2] Extre[3]
...
def get_data(p_e: bool):
    global S_xx, S_gx, S_gy, S_gz, S_ax, S_ay, S_az, gx, gy, gz, ax, ay, az, S_Loss, S_pk, LossPkt, factor, limite_XY
    S_pk = []
    S_xx = []
    S_gx = []
    S_gy = []
    ...
    while True:
        try:
            ...
            if not line:
                ...
                print("sx = ", S_xx[0], "-", S_xx[-1], " t = ", (S_xx[-1] - S_xx[0]) * (1 / 64), "s. No.M:", len(S_xx),
                      "\n")
                ...
                # Limites de la grafica amp_ind = [[-100, 100], [-0.8, 1.2]]    # Giroscopio[0] Acelerometro[1]
                b2 = max(max(S_gx), max(S_gy), max(S_gz), amp_ind[0][1]) + 5
                a2 = min(min(S_gx), min(S_gy), min(S_gz), amp_ind[0][0]) - 5
                b3 = max(max(S_ax), max(S_ay), max(S_az), amp_ind[1][1]) + 5 / 100
                a3 = min(min(S_ax), min(S_ay), min(S_az), amp_ind[1][0]) - 5 / 100
                limite_XY = [0, len(S_gx) - 1, a2, b2, a3, b3]
...
def animate():
    ...
    S_A.plot([S_xx[0], S_xx[-1]], [-tol_ce[0], -tol_ce[0]], "k--*",  linewidth=1)   # Cruce Umbral A
    S_A.plot([S_xx[0], S_xx[-1]], [tol_ce[0], tol_ce[0]], "k--*", linewidth=1)      # Cruce Umbral A
    ...
    # Limite de graficas
    S_G.set_xlim([S_xx[limite_XY[0]], S_xx[limite_XY[1]]])
    S_A.set_xlim([S_xx[limite_XY[0]], S_xx[limite_XY[1]]])

    S_G.set_ylim([limite_XY[2], limite_XY[3]])
    S_A.set_ylim([limite_XY[4], limite_XY[5]])
...
def mr_lim(expression):
    ...
    print("MR:", matriz_rot, S_xx[a1], S_xx[b1], abs(a1 - b1), abs(a1 - b1) * 1 / 64)
...
def xlimit_ajuste(expression):      # Visualizar y ajustar -> nuevo rango
    ...
    b2 = max(max(S_gx[a1:b1]), max(S_gy[a1:b1]), max(S_gz[a1:b1]), amp_ind[0][1]) + margen
    a2 = min(min(S_gx[a1:b1]), min(S_gy[a1:b1]), min(S_gz[a1:b1]), amp_ind[0][0]) - margen

    b3 = max(max(S_ax[a1:b1]), max(S_ay[a1:b1]), max(S_az[a1:b1]), amp_ind[1][1]) + margen / 100
    a3 = min(min(S_ax[a1:b1]), min(S_ay[a1:b1]), min(S_az[a1:b1]), amp_ind[1][0]) - margen / 100
...
def data_e(expression):
    ...
    if info[2] == "IFERN" and info[1] == 2:
        d = [[25, 550], [10, 250], [0.2, 1.4], [70, 550], 24, 3] # [25, 260], [10, 250], [0.2, 1.4], [70, 550], 24, 3]
        sec_ej = [1, -1]  # 1erSeg[0] 2doSeg[1], SegPos=+1, SegNeg=-1
...
def app_seg_auto(i):
    global matriz, tol_ce, d, posible_e, S_xx, S_gy, S_gz
    ...
    while not posible_e[0] and matriz and (posible_e[2] + 1 < len(S_xx)) and len(d) > 0:
        ...
        if k == posible_e[2]:
            ...
            if (s_obj[k] * s_obj[k - 1] < -(tol_ce[0] ** 2) and sum(cero_e) == 0) or \
                ...
                if len(cruc_e[0]) >= 1:
                    ...
                    if validacion[0] and validacion[1]:     # Si la Longitud y Area coinciden = SE VALIDO UN SEGMENTO
                        ...
                        print("Seg Valido:", S_xx[cruc_e[0][-1]], "-", S_xx[k], ". Validacion[NoM,A,V.Max/Min] = ",
                              validacion, r_segm[1])
                    else:
                        print("Seg Rechazado:", S_xx[cruc_e[0][-1]], "-", S_xx[k], ". Validacion[NoM,A,V.Max/Min] = ",
                              validacion, r_segm[1])
            ...
            if r_segm[1][0] == sec_ej[0] and r_segm[1][1] == sec_ej[1]:  # Sucesion deseada de segmentos
                ...
                if re_val[0]:
                    ...
                    indices = str([posible_e[1] - d[5], posible_e[2] + d[5]])

                    print("Posible ejercicio Validado: ", S_xx[r_segm[0][0][0]], "-", S_xx[r_segm[0][1][1]],
                    ...
                    S_G.plot([S_xx[posible_e[1]], S_xx[posible_e[1]]], [amp_ind[0][0], amp_ind[0][1]], "y--*")
                    S_G.plot([S_xx[posible_e[2]], S_xx[posible_e[2]]], [amp_ind[0][0], amp_ind[0][1]], "y--*")

                    S_A.plot([S_xx[posible_e[1]], S_xx[posible_e[1]]], [amp_ind[1][0], amp_ind[1][1]], "y--*")
                    S_A.plot([S_xx[posible_e[2]], S_xx[posible_e[2]]], [amp_ind[1][0], amp_ind[1][1]], "y--*")

                    # r_segm = [[[], []], [0, 0]]  # reset
                else:
                    print("Posible ejercicio Rechazado: ", S_xx[r_segm[0][0][0]], "-", S_xx[r_segm[0][1][1]],
                          "len: ", r_segm[0][1][1] - r_segm[0][0][0], " Inter:", r_segm[0][1][0] - r_segm[0][0][1],
                          "Info: ", re_val, "\n")
        ...
        if k == len(S_xx):
            print("SEGMENTADOR AUTOMATICO: FIN DE SEÑAL")
            imp_clasificados(k)
            break
...
def guardar_bd(event):
    ...
    if posible_e[0] or btn == GBD.label.get_text() and matriz and \
            (info[2] == "IAAC" or info[2] == "IFEC" or info[2] == "IFER" or info[2] == "IFERN"):
        ...
        for k in range(0, len(x)):
            ...
            if btn == C_BR.label.get_text() or btn == C_MR.label.get_text():
                posible_e[0] = False
                seg_automt.append([posible_e[1] - d[5], posible_e[2] + d[5], btn, datos[0], [x_data[0], x_data[-1], len(x_data)]])
...
def nombre_ejercicio(event):
    global info

    btn = ""
    if event.inaxes == iaac.ax:
        btn = iaac.label.get_text()

    if event.inaxes == ifer.ax:
        btn = ifer.label.get_text()
...
iaac = Button(plt.axes([0.96, 0.17, 0.03, 0.05]), nom_e[0], color="gray")
ifer = Button(plt.axes([0.96, 0.1, 0.03, 0.05]), nom_e[1], color="gray")

reset.on_clicked(re)
aplic.on_clicked(app_mr)
AppSA.on_clicked(app_seg_auto)
C_BR.on_clicked(guardar_bd)
C_MR.on_clicked(guardar_bd)
GBD.on_clicked(guardar_bd)
C_Imp.on_clicked(imp_clasificados)
iaac.on_clicked(nombre_ejercicio)
ifer.on_clicked(nombre_ejercicio)

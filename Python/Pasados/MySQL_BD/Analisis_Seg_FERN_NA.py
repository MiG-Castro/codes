...
config = [False, True, True, False, 0, 0, [], False]  # 1,4,5,7,8
SoloVer = False
crit_e = [2, "IFERN_B", "I"]  # Sensor[0], Ejercicio[1], Extremidad[2]
selec_señal = 3  # Gx = 0, Gy = 1, Gz = 2, Ax = 3, Ay = 4, Az = 5
tolerancia_cruce = [0.05, 5, 15]  # IntervaloCero[0] UmbralDistCruces[2] UmbralDist_IniFinEjer[3]
...
resultados = []
info_BD_tabla = ["nuevo_amanecer", "nuevo_ifer"]
...
def graficar(vector, n, on, recta):
    ...
    if on:
        for k in [0, 2, 4, 6]:
            #if k == 2 or k == 4:
            #    e1_A.plot(recta[0][k:k + 2], recta[1][k:k + 2], "r--*", linewidth=3)  # rectas por regresion lineal
            #else:
            #    e1_A.plot(recta[0][k:k + 2], recta[1][k:k + 2], "b--*", linewidth=3)

            if k == 0 or k == 4:
                e1_A.plot([recta[0][k], recta[0][k]], [-0.8, 0.8], "k--", linewidth=2)
            else:
                e1_A.plot([recta[0][k + 1], recta[0][k + 1]], [-0.8, 0.8], "k--", linewidth=2)
...
def pendiente(cero, x, gy):
    ...
    if sum(gy[temp[3][0]:temp[3][1]]) < -1:  # Parte negativa
        ind[1] = gy.index(min(gy[temp[3][0]:temp[3][1]]))
        ind[4] = gy.index(max(gy[temp[3][2]:temp[3][3]]))
    elif sum(gy[temp[3][0]:temp[3][1]]) > 1:  # Parte positiva

...
# Imprimir[0], IIR[1], graficar[2], T=Unico F=Bucle[3], ID_ej[4], InicioBulce[5]
config = [True, True, True, False, 6, 0]
...
def busqueda(ejercicio, sensor):            # Busqueda y almacenamiento de todos los ejercicios coincidentes por sensor
    print("Parametros de busqueda: ", ejercicio, ", ", str(sensor), end=". ")
    resultado = False
    lista = []
    n = [0, 0]
    for a in Ejercicios_Filas:
        if Ejercicios_Filas[n[0]][1] == ejercicio and Ejercicios_Filas[n[0]][3] == sensor:
...
def graficar(vector, n, on, recta):
    ...
    e1_G.plot([vector[1][0], vector[1][len(vector[1]) - 1]], [tolerancia_cruce[1], tolerancia_cruce[1]], "k--")
...
def cruce_cero(x, vector):
    ...
    if tolerancia_cruce[0] < vector[0] < tolerancia_cruce[1]:       # Muestra en el intervalo de tolerancia para "cero"?
        cero[0] = True

    for k in arange(1, len(vector)):            # Cruces por cero
        if tolerancia_cruce[0] < vector[k] < tolerancia_cruce[1]:   # En intervalo de tolerancia "cero"
            cero[1] = True
            # print(vector[k], x[k], " intervalo")

        if (vector[k] * vector[k-1]) < (tolerancia_cruce[0] * tolerancia_cruce[1]):     # Paso "+ -> -" o "- -> +"
            cruces[0].append(k)
            # print(vector[k], "(", x[k], ") x ", vector[k-1], "(", x[k-1], ") =", (vector[k] * vector[k-1]), x[k], " Paso")

        if cero[0] != cero[1]:      # Cambio
            if len(cruces[0]) == 0:
                cruces[0].append(k)
            elif (k - cruces[0][-1]) > 5:
                cruces[0].append(k)
...
def ajusteCruceGy(x, disCruce, cruces):
    ...
    if (cruces[0] - x[0]) > 30:                     # Cruce faltante al inicio
    ...
    if (x[-1] - cruces[-1]) > 30:                   # Cruce faltante al final
...
def pendiente(cero, x, gy):
    ...
    temp[3] = [x.index(cero[1][temp[2][0]]), x.index(cero[1][temp[2][1]]), x.index(cero[1][temp[2][2]]), x.index(cero[1][temp[2][3]])]
    ...
    if sum(gy[temp[3][0]:temp[3][1]]) < -1000:                      # Parte negativa
        ind[1] = gy.index(min(gy[temp[3][0]:temp[3][1]]))
        ind[4] = gy.index(max(gy[temp[3][2]:temp[3][3]]))
    elif sum(gy[temp[3][0]:temp[3][1]]) > 1000:                     # Parte positiva
...
if not config[3]:                                       # BUCLE
    e_selec = busqueda("AA", 3)  # lista ejercicios -> [Ejercicio[0] - Sensor[1] - Fecha[2] - ID[3] - Fila[4]] - No[5]
    print(" \nAnalisis de ejercicios ...", end="")
    resultados = []
    for n in arange(config[5], len(e_selec)):
        e1 = sf(e_selec[n][4])
        ...
        ceros = cruce_cero(e1[1], e1[3])                # Deteccion de cruces
        print("\n", n, ceros)
        ...
        mb, recta = pendiente(ceros, e1[1], e1[3])      # Obtencion de pendientes Gy
        # print("CEROS: ", ceros)

        print("mb= ", mb[0], "\nRecta= ", recta)

        if config[2]:                                   # Grafica de ejercicio
        ...
        # resultados.append([e_selec[n], len(e1[1])])
else:                                                   # Ejercicio unico
    print(" \nAnalisis de ejercicio ...", end="")
    resultados = []
    ...
    resultados.append(e1[0])                            # Informacion
    resultados.append([len(e1[1])])                     # Duracion
    n = len(resultados)
    for k in arange(2, 8):                              # Max y min Gx[2], Gy[3], Gz[4], Ax[5], Ay[6], Az[7]
        v = [0, 0]
        v[0] = max(e1[k])
        v[1] = min(e1[k])
        if k == 2:
            resultados.append([v[0]])
            resultados.append([v[1]])
            resultados.append([abs(e1[k].index(v[0]) - e1[k].index(v[1]))])
        else:
            resultados[n].append(v[0])
            resultados[n + 1].append(v[1])
            resultados[n + 2].append(abs(e1[k].index(v[0]) - e1[k].index(v[1])))

    n = len(resultados)
    for k in arange(2, 8):                              # Energia Gx[2], Gy[3], Gz[4], Ax[5], Ay[6], Az[7]
        if k == 2:
            resultados.append([energia(e1[k])])
        else:
            resultados[n].append(energia(e1[k]))

    # Pendiente

    """
    for k in arange(2, 5):                              # Indices y Distancia entre cruces por cero
        print("\n", cruce_cero(e1[1], e1[k]), end="")
    """
    print("\n", cruce_cero(e1[1], e1[2]))
...
if config[0]:
    print("Resultados: ")
    #for n in arange(0, len(resultados)):
    #    print(resultados[n])
    print(resultados)

"""
# version 1 cruce por cero, detecta cambios de un valor positivo a un valor de 0 o menor
def cruce_cero(x, vector):
    cruces = [[], []]                           # Indice[0] - longitud[1]
    positivo = [False, False]                   # ValorPasado[0] - ValorActual[1]
...
# PENDIENTE NO FUNCIONA
    for k in [0, 1, 3, 4]:
        parametros = [0, 0, 0, 0, 0, len(x[ind[k]:ind[k+1]])]       # promX[0] promY[1] varX[2] varY[3] covXY[4] n[5]
        print("Arreglo: ", ind[k], ind[k+1], len(x[ind[k]:ind[k+1]]))

        for k2 in arange(ind[k], ind[k + 1] + 1):
            parametros[0] = parametros[0] + x[k2]
            parametros[1] = parametros[1] + gy[k2]
            #print(k2, x[k2], parametros[0], gy[k2], parametros[1])
            print(x[k2])

        parametros[0] = parametros[0] / parametros[5]               # Prom X
        parametros[1] = parametros[1] / parametros[5]               # Prom Y
        print(parametros)

        for k2 in arange(ind[k], ind[k + 1] + 1):
            parametros[2] = parametros[2] + (x[k2] - parametros[0]) ** 2
            parametros[3] = parametros[3] + (gy[k2] - parametros[1]) ** 2
            parametros[4] = parametros[4] + (x[k2] - parametros[0]) * (gy[k2] - parametros[1])
            print(gy[k2])

        parametros[2] = parametros[2] / parametros[5]               # Var X
        parametros[3] = parametros[3] / parametros[5]               # Var Y
        parametros[4] = parametros[4] / parametros[5]               # Cov XY

        print(parametros)
        y[0].append(parametros[4] / parametros[2])                  # m
        y[1].append(parametros[1] - y[0][-1] * parametros[0])       # b
        print(parametros, y)
"""
...
def animate():
    ...
    if sum(matriz_rot) > 0:
        S_G.plot([S_xx[matriz_rot[0]], S_xx[matriz_rot[0]]], [-50, 50], "c--*")
        S_G.plot([S_xx[matriz_rot[1]], S_xx[matriz_rot[1]]], [-50, 50], "c--*")
    ...
    for k in range(0, len(seg_manual[0])):
        if seg_manual[1][k]:
            S_G.plot([seg_manual[0][k], seg_manual[0][k]], [-100, 100], "r--*", picker=True, pickradius=1)
        else:
            S_G.plot([seg_manual[0][k], seg_manual[0][k]], [-100, 100], "k--*", picker=True, pickradius=1)

    for k in range(0, len(ind_precargados)):
        S_G.plot([S_xx[ind_precargados[k][0]], S_xx[ind_precargados[k][0]]], [-100, 100], "y--*", picker=True, pickradius=1)
        S_G.plot([S_xx[ind_precargados[k][1]], S_xx[ind_precargados[k][1]]], [-100, 100], "y--*", picker=True, pickradius=1)
...
def onpick(event):
    ...
    if txt_b == "NoM":
        ...
        try:
            ...
            ism = ism[-2:]
            S_G.plot([ism[-1], ism[-1]], [-100, 100], "g--*")
...
def indice_manual(expression):
    ...
    try:
        if txt_b == "NoM":
            ...
            S_G.plot([S_xx[ism[0]], S_xx[ism[0]]], [-100, 100], "g--*")
            S_G.plot([S_xx[ism[1]], S_xx[ism[1]]], [-100, 100], "g--*")
    except:

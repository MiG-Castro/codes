...
tabla = "ejercicios_prueba"
ejercicio = "05-04-2022_08-44-11_IAAB_s2.txt"
paciente = [89, "10a 7m"]
Extremidad = "D"
ind_precargados = [[234, 400], [50, 111]]
...
def animate():
    ...
    if sum(matriz_rot) > 0:
        S_A.plot([S_xx[matriz_rot[0]], S_xx[matriz_rot[0]]], [-0.5, 1.2], "c--*")
        S_A.plot([S_xx[matriz_rot[1]], S_xx[matriz_rot[1]]], [-0.5, 1.2], "c--*")
    ...
    for k in range(0, len(seg_manual[0])):
        if seg_manual[1][k]:
            S_A.plot([seg_manual[0][k], seg_manual[0][k]], [-1, 1], "r--*", picker=True, pickradius=1)
        else:
            S_A.plot([seg_manual[0][k], seg_manual[0][k]], [-1, 1], "k--*", picker=True, pickradius=1)

    for k in range(0, len(ind_precargados)):
        S_A.plot([S_xx[ind_precargados[k][0]], S_xx[ind_precargados[k][0]]], [-1, 1], "y--*", picker=True, pickradius=1)
        S_A.plot([S_xx[ind_precargados[k][1]], S_xx[ind_precargados[k][1]]], [-1, 1], "y--*", picker=True, pickradius=1)
...
def data_save_mysql(expression):
    ...
    if matriz and (info[4] == "D" or info[4] == "I") and info[0][0] > 0:
        ...
        if txt_b == "NoM":
            print(datos[-1][0], info, ism, len(S_xx[x[0]:x[1]]), [S_xx.index(ism[0]), S_xx.index(ism[1])], )
        elif txt_b == "Ind":
            print(datos[-1][0], info, ism, len(S_xx[x[0]:x[1]]), [S_xx[ism[0]], S_xx[ism[1]]])
...
def data_save_PreCarg_mysql(expression):
    ...
    if matriz and txt_b == "Ind" and len(ind_precargados) > 0 and (info[4] == "D" or info[4] == "I") and info[0][0] > 0:
        ...
        for k in range(0, len(ind_precargados)):
            ...
            print(datos[-1][0], info, x, len(S_xx[x[0]:x[1]]), [S_xx[x[0]], S_xx[x[1]]])
            datos = []
...
def indice_manual(expression):
    ...
    try:
        if txt_b == "NoM":
            ...
            S_A.plot([S_xx[ism[0]], S_xx[ism[0]]], [-1, 1], "g--*")
            S_A.plot([S_xx[ism[1]], S_xx[ism[1]]], [-1, 1], "g--*")
    except:
...
check1 = CheckButtons(plt.axes([0.95, 0.75, 0.1, 0.2]), s_lab[0], s_lab[1])

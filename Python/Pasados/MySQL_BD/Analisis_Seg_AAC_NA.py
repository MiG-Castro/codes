...
config = [True, True, True, True, 164, 0, [], False]  # 160 +-1 inter 922    706
SoloVer = True
crit_e = [2, "IAACM", "D"]  # Sensor[0], Ejercicio[1], Extremidad[2]
...
resultados = []
info_BD_tabla = ["nuevo_amanecer", "ejercicios_segmentador"]  # e_seg_auto
...
def graficar(vector, n, on, recta):
    ...
    e1_G.plot(vector[1], (vector[3]), "m-", linewidth=3, label="Señal de interés")  # "r*-", --. -d
    ...
    if on:
        for k in [0, 2, 4, 6]:
            ...
            if k == 2 or k == 4:
                e1_G.plot(recta[0][k:k + 2], recta[1][k:k + 2], "r--*", linewidth=3)
                if k == 2:
                    e1_G.plot(recta[0][k:k + 2], recta[1][k:k + 2], "r--*", linewidth=3, label="Recta positiva")  # rectas por regresion lineal
            else:
                if k == 0:
                    e1_G.plot(recta[0][k:k + 2], recta[1][k:k + 2], "b--*", linewidth=3, label="Recta negativa")
                e1_G.plot(recta[0][k:k + 2], recta[1][k:k + 2], "b--*", linewidth=3)
            """
            if k == 0 or k == 6:
                if k == 0:
                    e1_G.plot([recta[0][k] - 3, recta[0][k] - 3], [-100, 100], "b--", linewidth=2, label="Señal Segmentada")
                if k == 6:
                    e1_G.plot([recta[0][k + 1] + 3, recta[0][k + 1] + 3], [-100, 100], "b--", linewidth=2)
    ...
    # e1_G.plot([vector[1][0], vector[1][len(vector[1]) - 1]], [-tolerancia_cruce[0], -tolerancia_cruce[0]], "r--", label="Umbral")
    # e1_G.plot([vector[1][0], vector[1][len(vector[1]) - 1]], [tolerancia_cruce[0], tolerancia_cruce[0]], "r--")
    e1_G.plot([vector[1][0], vector[1][len(vector[1]) - 1]], [0, 0], "k-")
    ...
    # e1_G.legend(loc='upper left', fontsize=14)
    ...
    e1_G.grid(True)
    e1_A.grid(True)
    e1_G.set_xlim(vector[1][0] - 10, vector[1][0] + 270)
    e1_G.set_ylim(-70, 70)
    plt.show()

...
config = [False, True, True, False, 92, 0, [], False]
SoloVer = False
crit_e = [3, "IFECB", "D"]  # Sensor[0], Ejercicio[1], Extremidad[2]
tolerancia_cruce = [0.025, 5, 50]        # IntervaloCero[0] UmbralDistCruces[2] UmbralDist_IniFinEjer[3]
...
resultados = []
info_BD_tabla = ["nuevo_amanecer", "ejercicios_segmentador"]  # e_seg_auto ejercicios_segmentador
...
def busqueda(sensor, ejercicio, lado):  # Busqueda y almacenamiento de todos los ejercicios coincidentes por sensor
    ...
    for a in Ejercicios_Filas:
        if Ejercicios_Filas[n[0]][5] == ejercicio and Ejercicios_Filas[n[0]][4] == sensor and \
                Ejercicios_Filas[n[0]][6] == lado:
...
def graficar(vector, n, on, recta):
    ...
    if on:
        ...
        # e1_A.plot(recta[0][0:2], recta[1][0:2], "b--*", linewidth=3)
        # e1_A.plot(recta[0][2:], recta[1][2:], "r--*", linewidth=3)

        # Delimitacion de segmento
        minimo = min(vector[6][vector[1].index(recta[0][0]):vector[1].index(recta[0][-1])])
        e1_A.plot([recta[0][0], recta[0][0]], [-0.2, 1], "k--", linewidth=2)
        e1_A.plot([recta[0][-1], recta[0][-1]], [-0.2, 1], "k--", linewidth=2)

        #e1_A.plot([recta[0][0], recta[0][0]], [minimo, tolerancia_cruce[0] + 0.1], "k--", linewidth=2)
        #e1_A.plot([recta[0][-1], recta[0][-1]], [minimo, tolerancia_cruce[0] + 0.1], "k--", linewidth=2)

    e1_G.plot(vector[1], (vector[2]), "b-", label="Gx")
    e1_G.plot(vector[1], (vector[3]), "m-", label="Gy")  # "r*-", --. -d
    e1_G.plot(vector[1], (vector[4]), "g-", label="Gz")

    e1_A.plot([vector[1][0], vector[1][len(vector[1]) - 1]], [-tolerancia_cruce[0] + 1, -tolerancia_cruce[0] + 1], "k--")
    e1_A.plot([vector[1][0], vector[1][len(vector[1]) - 1]], [tolerancia_cruce[0] + 1, tolerancia_cruce[0] + 1], "k--")

    #e1_A.plot([vector[1][0], vector[1][len(vector[1]) - 1]], [-tolerancia_cruce[0], -tolerancia_cruce[0]], "k--")
    #e1_A.plot([vector[1][0], vector[1][len(vector[1]) - 1]], [tolerancia_cruce[0], tolerancia_cruce[0]], "k--")
...
def ajusteCruceAy(x, disCruce, cruces):
    global tolerancia_cruce
    new_disCruce = []
    new_cruces = []
...
def pendiente_Ay(cero, x, gy):
    ...
    temp[2] = [temp[1][0], temp[1][0] + 1]

    # Indices de segmetos ordenados correspondientes al eje X
    temp[3] = [x.index(cero[1][temp[2][0]]), x.index(cero[1][temp[2][1]])]
    temp[4] = [cero[1][temp[2][0]], cero[1][temp[2][1]]]

    # Calculo de las pendientes de los segmentos usando metodo de minimos cuadrados ************************************
    ind = [temp[3][0], 0, temp[3][1]]
    ind[1] = gy.index(min(gy[temp[3][0]:temp[3][1]]))
    # if sum(gy[temp[3][0]:temp[3][1]]) < -100:  # Parte negativa
    #    ind[1] = gy.index(min(gy[temp[3][0]:temp[3][1]]))

    for k in [0, 1]:  # [0, 1, 3, 4]
...
for n in vector_e:  # EJECUCION DE OPERACIONES #########################################################################
    ...
    ################################################################################################################
    # xx = e1[1]  Gx = e1[2]  Gy = e1[3]  Gz = e1[4]  Ax = e1[5]  Ay = e1[6]  Az = e1[7]

    print(n)                                # INFORMACION EJERCICIO
    print(e1[0][0])
    if not config[0]:
        print("")

    for k in arange(0, len(e1[1])):         # offset Ay
        e1[6][k] = e1[6][k] - 1
    ...
    if not SoloVer:
        ...
        ceros = cruce_cero(e1[1], e1[6])  # Deteccion de cruces Ay = e1[6]
        # print(ceros)

        k0 = 0
        for k1 in arange(0, len(ceros[0])):  # Rev cruces
            if ceros[0][k1] > 50:
                k0 = k0 + 1

        if k0 < 1:  # Ajuste cruces Ay
            ceros = ajusteCruceAy(e1[1], ceros[0], ceros[1])
            # print("Ajuste: ", ceros)

        recta = []
        mb, recta, seg = pendiente_Ay(ceros, e1[1], e1[6])  # Obtencion de Eq Recta y segmentos
        # print("mb= ", mb[0], "\nRecta= ", recta, "\nSegmento: ", seg)

        resultados.append(e1[6][seg[1]])  # Min Ay

        resultados.append(mb[0][0])  # Pendientes segmento
        resultados.append(mb[0][1])

        resultados.append(seg[1] - seg[0] + 1)  # Longitud Parcial Segmento
        resultados.append(seg[2] - seg[1] + 1)

        resultados.append(seg[2] - seg[0] + 1)  # Longitud total de segmento
        resultados.append(len(e1[6]))  # Longitud TOTAL DEL EJERCICIO

        resultados.append(sum(e1[6][seg[0]:seg[2] + 1]))  # Area segmento
        resultados.append(energia(e1[6][seg[0]:seg[2] + 1]))  # Energia primer segmento
        resultados.append(energia(e1[6]))  # Energia total del ejercicio

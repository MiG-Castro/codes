...
config = [
    True,  # [0] Imprimir resultados
    False,  # [1] Aplicar IIR
    True,  # [2] Graficar ejercicios
    True,  # [3] EjercicioUnico=TRUE <-> BucleEjercicios=False
    692,  # [4] ID Ejercicio Unico
    0,  # [5] No. Pyton de inicio bucle
    [],  # [6] No. Pyton de ejercicios a omitir
    True]  # [7] Guardar datos extraidos en archivo Weka
...
texto_extra = ["_mNo_ETp_A_SinImpar"]
...
def get_caracteristicas(ejercicio):
    ...
    if len(ejercicio) == 6:
        ...
        # Energia de la señal
        ETG = np.sum(gxf ** 2) + np.sum(gyf ** 2) + np.sum(gzf ** 2)
        Egx = np.sum(gxf ** 2) / ETG
        Egy = np.sum(gyf ** 2) / ETG
        Egz = np.sum(gzf ** 2) / ETG

        ETA = np.sum(axf ** 2) + np.sum(ayf ** 2) + np.sum(azf ** 2)
        Eax = np.sum(axf ** 2) / ETA
        Eay = np.sum(ayf ** 2) / ETA
        Eaz = np.sum(azf ** 2) / ETA
        ...
        # recopilacion
        caracteristicas = [ETG, Egx, Egy, Egz, ETA, Eax, Eay, Eaz, Agx, Agy, Agz, Aax, Aay, Aaz, Mgx, Mgy, Mgz, Max,

                            ...
                            if len(S_xx) > 1 and S_pk[-1] != (S_pk[-2] + 1):
                                ...
                                while LossPkt[3] > 0:
                                    for k in [1, 2, 3]:
                                        ...
                                        S_gx.append(round(m[0] * S_xx[-1] + b[0], redondeo))
                                        S_gy.append(round(m[1] * S_xx[-1] + b[1], redondeo))
                                        S_gz.append(round(m[2] * S_xx[-1] + b[2], redondeo))
                                        S_ax.append(round(m[3] * S_xx[-1] + b[3], redondeo))
                                        S_ay.append(round(m[4] * S_xx[-1] + b[4], redondeo))
                                        S_az.append(round(m[5] * S_xx[-1] + b[5], redondeo))
...
def guardar_txt(i):
    global S_xx, S_gx, S_gy, S_gz, S_ax, S_ay, S_az

    file = open(nombre + ejercicio, "w+")

    for k in range(len(S_gx)):

        temporal = [S_gx[k], S_gy[k], S_gz[k], S_ax[k], S_ay[k], S_az[k]]  # S_xx[k],
        file.writelines(str(temporal).replace("[", "").replace("]", "\n"))
    file.close()

    print("Archivo:nombre + ejercicio")


txtbox0 = fig.add_axes([0.04, 0.005, 0.15, 0.05])
txtbox1 = fig.add_axes([0.29, 0.005, 0.15, 0.05])
txtbox2 = fig.add_axes([0.54, 0.005, 0.2, 0.05])

text_box0 = TextBox(txtbox0, "xlim:")
text_box1 = TextBox(txtbox1, "MR:")
text_box2 = TextBox(txtbox2, "Ind:")
...
G_txt = Button(plt.axes([0.75, 0.005, 0.04, 0.05]), "G-txt", color="gray")

reset.on_clicked(re)
aplic.on_clicked(app_mr)
G_txt.on_clicked(guardar_txt)

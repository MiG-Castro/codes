...
info = [paciente, int(temp_info[2]), "IFEC", Extremidad]    # Paciente[0] Sensor[1] Ejercicio[2] Extre[3]
...
def animate():
    global S_xx, S_gx, S_gy, S_gz, S_ax, S_ay, S_az, gx, gy, gz, ax, ay, az, cambio, matriz_rot, limite_XY
    ...
    S_A.plot([S_xx[0], S_xx[-1]], [-tol_ce[0], -tol_ce[0]], "k--*",  linewidth=1)   # Cruce Umbral Acelerometro
    S_A.plot([S_xx[0], S_xx[-1]], [tol_ce[0], tol_ce[0]], "k--*", linewidth=1)      # Cruce Umbral Acelerometro
...
def data_e(expression):
    ...
    if info[2] == "IFEC" and info[1] == 3:
        d = [[30, 310], [-10, -200], [0.3, 1.4], [120, 380], 0, 35]
...
def app_seg_auto(i):
    ...
    while not posible_e[0] and matriz and posible_e[2] + 1 < len(S_xx) and len(d) > 0:
            ...
            if (s_obj[k] * s_obj[k - 1] < -(tol_ce[0] ** 2) and sum(cero_e) == 0) or \
                ...
                if len(cruc_e[0]) >= 1:     # ANALISIS DE SEGMENTO
                    ...
                    if validacion[0] and validacion[1]:     # REGISTRO DE SEGMENTO VALIDO
                        ...
                        print("Seg Rechazado:", S_xx[cruc_e[0][-1]], "-", S_xx[k], ". Validacion[NoM,A,V.Max/Min] = ", validacion)
        ...
        if k == len(S_xx):
            print("FIN DE ANALISIS SEGMENTADOS")
            imp_clasificados(k)
            break

...
muestras_x_pkt = 1                                          # Muestras x paquete
...
# Archivo a leer
file = "Pruebas_BLE/1m_60HzTx/5m/2025-04-01_16-14-09_None-2.bin"
...
if unpack:
    ...
    # Perdida de paquetes
    print(f"Tasa de exito: {((pkt_recibidos / paquetes_sin_perdida) * 100):.2f}", end=" %, ")
    loss_pkts = []
    for k in range(len(sec) - 1):
        if sec[k + 1] - sec[k] > 1:
            perida = sec[k + 1] - sec[k] - 1
            loss_pkts.append(perida)
    if len(loss_pkts) == 0:
        loss_pkts.append(0)
    print(f'Paquetes pedidos: {sum(loss_pkts)}, promedio de perdidas secuenciales: {(sum(loss_pkts)/len(loss_pkts)):.2f}\n')

    if sec[-1] != 35999:
        print("El archivo no contiene todos los paquetes esperados")
        print("Ultimo paquete:", sec[-1], ", Paquetes esperados:", 36000, 
              "Diferencia: ", 36000 - (sec[-1] + 1), ", Perdidas totales: ", 36000 - (sec[-1] + 1) + sum(loss_pkts), "\n")

...
plt.subplots_adjust(left=0.05, top=0.95, bottom=0.07, right=1.4, wspace=0, hspace=0.05)
...
GE = fig.add_subplot(2, 2, 2)
AG = fig.add_subplot(2, 2, 4)
GE.axis('off')
AG.axis('off')

GE.text(0.09, 0.85, 'Energia Gyro:', fontsize=24)
GE.text(0.03, 0.58, 'Eje X:', fontsize=17)
GE.text(0.03, 0.35, 'Eje Y:', fontsize=17)
GE.text(0.03, 0.11, 'Eje Z:', fontsize=17)
AG.text(0.21, 0.70, 'Alineación\nVector Gravedad', ha='center', fontsize=24)

actualizacion_ms = 100
m_pos = 64
...
lengthVector = 120      # Numero de muestras a graficar, 1 muestra = 15.625 ms
...
def animate(i):
    ...
    EGx.label.set_text(str(int(E[0])))
    EGy.label.set_text(str(int(E[1])))
    EGz.label.set_text(str(int(E[2])))

    e_min = 500
    maximo = E.index(max(E))
    if maximo == 0 and E[0] > e_min:
        EGx.ax.patch.set_facecolor('orange')
    else:
        EGx.ax.patch.set_facecolor('none')

    if maximo == 1 and E[0] > e_min:
        EGy.ax.patch.set_facecolor('orange')
    else:
        EGy.ax.patch.set_facecolor('none')

    if maximo == 2 and E[0] > e_min:
        EGz.ax.patch.set_facecolor('orange')
    else:
        EGz.ax.patch.set_facecolor('none')
    ...
    if umbral[0] <= abs(G[0]) <= umbral[1]:
        Eje.ax.patch.set_facecolor('blue')
        if G[0] < 0:
            Eje.label.set_text('-X')
        else:
            Eje.label.set_text('+X')

    elif umbral[0] <= abs(G[1]) <= umbral[1]:
        Eje.ax.patch.set_facecolor('orange')
        if G[1] < 0:
            Eje.label.set_text('-Y')
        else:
            Eje.label.set_text('+Y')

    elif umbral[0] <= abs(G[2]) <= umbral[1]:
        Eje.ax.patch.set_facecolor('green')
        if G[2] < 0:
            Eje.label.set_text('-Z')
        else:
            Eje.label.set_text('+Z')

    else:
        Eje.ax.patch.set_facecolor('none')
        Eje.label.set_text('')
...
Loss = Button(plt.axes([0.07, 0.01, 0.83, 0.04]), Total_E, color="none")
Loss.on_clicked(filtro_OF)

EGx = Button(plt.axes([0.83, 0.75, 0.15, 0.07]), "", color="none")
EGy = Button(plt.axes([0.83, 0.65, 0.15, 0.07]), "", color="none")
EGz = Button(plt.axes([0.83, 0.55, 0.15, 0.07]), "", color="none")
Eje = Button(plt.axes([0.79, 0.12, 0.15, 0.2]), "", color="none")

EGx.label.set_fontsize(22)
EGy.label.set_fontsize(22)
EGz.label.set_fontsize(22)
Eje.label.set_fontsize(70)

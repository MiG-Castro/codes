...
plt.subplots_adjust(left=0.03, top=0.95, bottom=0.07, right=0.98, wspace=0.1, hspace=0.05)

S2_A = fig.add_subplot(2, 3, 1)
S2_G = fig.add_subplot(2, 3, 2)
imag = fig.add_subplot(1, 3, 3)
...
AG = fig.add_subplot(2, 3, 4)
GE = fig.add_subplot(2, 3, 5)
...
def animate(i):
    global S2_xx, S2_GA, S2_min, lengthVector, ultimo, Total_E, ancho, renglones
    ...
    if len(S2_xx) > 1 and S2_xx[-1] > (ultimo[0]+2):
        ...
        if renglones < 256:
            empty_img[renglones:renglones + 3, :, :] = img[renglones:renglones + 3, :, :]  # Copiar la fila i de la imagen original
            imag.imshow(empty_img)

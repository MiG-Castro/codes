...
fig = plt.figure(layout="constrained", figsize=(12, 6)) #figsize=(12, 6)
gs = GridSpec(2, 5, figure=fig)
gs.update(left=0.3)
# plt.subplots_adjust(left=0.05, top=0.95, bottom=0.07, right=1.4, wspace=0, hspace=0.05)

S2_A = fig.add_subplot(gs[0, 0:2])
S2_G = fig.add_subplot(gs[0, 2:4])
brra = fig.add_subplot(gs[:, -1])

S2_G.get_xaxis().set_visible(False)
S2_A.get_xaxis().set_visible(False)

# brra.get_xaxis().set_visible(False)
# brra.get_yaxis().set_visible(False)

AG = fig.add_subplot(gs[1, 0:2])
GE = fig.add_subplot(gs[1, 2:4])
...
GE.text(0.5, 0.8, 'Energia Giroscopio', ha='center', fontsize=tam_l[2])
AG.text(0.5, 0.8, 'Alineación Vector Gravedad', ha='center', fontsize=tam_l[2])

GE.text(0.03, 0.58, 'Eje X:', fontsize=tam_l[0], fontweight='bold', color='blue')
GE.text(0.03, 0.35, 'Eje Y:', fontsize=tam_l[0], fontweight='bold', color='orange')
GE.text(0.03, 0.11, 'Eje Z:', fontsize=tam_l[0], fontweight='bold', color='green')
...
gravedad = AG.text(0.5, 0.3, '', fontsize=120, ha='center', fontweight='bold', color='black')
...
def animate(i):
    ...
    if len(S2_xx) > 1 and S2_xx[-1] > (ultimo[0]+2):
        ...
        # Titulos
        S2_G.set_title('Giroscopio', fontsize=18)
        S2_A.set_title('Accelerometro',  fontsize=18)

        S2_G.grid(True)
        S2_A.grid(True)

        # Barra
        brra.set_title('hola', fontsize=24, color="black")
        brra.bar([50], [20], width=0.2, color='blue')
        brra.set_ylim([0, 100])
        brra.set_ylabel('Poder')
...
ani = animation.FuncAnimation(fig, animate, interval=actualizacion_ms, cache_frame_data=False, blit=True)

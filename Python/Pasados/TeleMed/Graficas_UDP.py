...
import time
import socket
from statistics import mean, stdev  # Importar las funciones mean y stdev para el cálculo de la media y desviación estándar
...
def main():
    ...
    def update_plot():
        ...
        try:
            ...
            if data != 'Q':
                ...
                if num_samples > 1:
                    jitter = abs(current_delay - (total_delay / num_samples)) # Se calcula jitter como la diferencia de los paquetes y media de estos
                    total_jitter += jitter 
    ...
    def on_closing():
        ...
        if all_jitters:
            ...
            jitter_stdev = stdev(all_jitters) * 1000 if len(all_jitters) > 1 else 0  # Calcula la desviación estándar si hay más de una muestra, de lo contrario, establece en 0
        ...
        if num_samples > 0:
            ...
            delay_stdev = stdev(all_delays) * 1000 if num_samples > 1 else 0  # Calcula la desviación estándar si hay más de una muestra, de lo contrario, establece en 0
...
def simulation(q):
    ...
    puerto = 3333
    ip = "192.168.0.8"  # Reemplaza con tu IP local
